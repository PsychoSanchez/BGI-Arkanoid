...
void drawbrick(int i, int j,Scene *Sc)
{
		...
		case 1: setcolor(WHITE);
				rectangle(Sc->AllBricks[i][j].lx,Sc->AllBricks[i][j].ly,Sc->AllBricks[i][j].lx+41,Sc->AllBricks[i][j].ly+15);
				rectangle(Sc->AllBricks[i][j].lx+2,Sc->AllBricks[i][j].ly+2,Sc->AllBricks[i][j].lx+41-2,Sc->AllBricks[i][j].ly+15-2);
                 break;
			//Блок с бонусными очками
		case 2: setcolor(YELLOW);
				rectangle(Sc->AllBricks[i][j].lx,Sc->AllBricks[i][j].ly,Sc->AllBricks[i][j].lx+41,Sc->AllBricks[i][j].ly+15);
				rectangle(Sc->AllBricks[i][j].lx+2,Sc->AllBricks[i][j].ly+2,Sc->AllBricks[i][j].lx+41-2,Sc->AllBricks[i][j].ly+15-2);
				setcolor(WHITE);
                break;
				//Блок с бонусными шариками
		case 3: setcolor(RED);
				rectangle(Sc->AllBricks[i][j].lx,Sc->AllBricks[i][j].ly,Sc->AllBricks[i][j].lx+41,Sc->AllBricks[i][j].ly+15);
				rectangle(Sc->AllBricks[i][j].lx+2,Sc->AllBricks[i][j].ly+2,Sc->AllBricks[i][j].lx+41-2,Sc->AllBricks[i][j].ly+15-2);
				setcolor(WHITE);
                break; 
		case 4: setcolor(LIGHTBLUE);
				rectangle(Sc->AllBricks[i][j].lx,Sc->AllBricks[i][j].ly,Sc->AllBricks[i][j].lx+41,Sc->AllBricks[i][j].ly+15);
				rectangle(Sc->AllBricks[i][j].lx+2,Sc->AllBricks[i][j].ly+2,Sc->AllBricks[i][j].lx+41-2,Sc->AllBricks[i][j].ly+15-2);
				setcolor(WHITE);
                break; 
		case 9: setcolor(LIGHTGRAY);
				rectangle(Sc->AllBricks[i][j].lx,Sc->AllBricks[i][j].ly,Sc->AllBricks[i][j].lx+41,Sc->AllBricks[i][j].ly+15);
				floodfill(Sc->AllBricks[i][j].lx+10,Sc->AllBricks[i][j].ly+7, LIGHTGRAY);
				setcolor(WHITE);
		}
}
		...
		if(Sc->Balls[i].radius+Sc->Balls[i].y>=Sc->height)
			{
				...
				{
					Sc->lifes-=1;
					if(Sc->lifes==2) outtextxy(5,450,"2 lifes remaining");
					if(Sc->lifes==1) outtextxy(5,450,"1 life remaining");
					Sc->num++;
					delay(500);
					Sc->Balls[0].x=300;
					Sc->Balls[0].y=300;
...
void PlatformCheck(Scene *Sc)
{
	for(int i=0; i<Sc->num; i++)
	{
		 ...
		 {
			 //Разные повороты под разными углами
			 if(Sc->Balls[i].x>Sc->paddleX-40 && Sc->Balls[i].x<Sc->paddleX-30)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=-2;
			 }
			  if(Sc->Balls[i].x>Sc->paddleX-30 && Sc->Balls[i].x<Sc->paddleX-20)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=1.6;
			 }
			 if(Sc->Balls[i].x>Sc->paddleX-20 && Sc->Balls[i].x<Sc->paddleX-10)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=1.3;
			 }
			  if(Sc->Balls[i].x>Sc->paddleX-10 && Sc->Balls[i].x<Sc->paddleX+10)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=0.6;
			 }
			 if(Sc->Balls[i].x>Sc->paddleX+10 && Sc->Balls[i].x<Sc->paddleX+20)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=1.3;
			 }
			  if(Sc->Balls[i].x>Sc->paddleX+20 && Sc->Balls[i].x<Sc->paddleX+30)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=1.6;
			 }
			  if(Sc->Balls[i].x>Sc->paddleX+30 && Sc->Balls[i].x<Sc->paddleX+40)
			 {
				Sc->Balls[i].vy*=-1;
				Sc->Balls[i].vx*=-2;
			 }
			  //Максимальная скорость шарика, а то порой летает как бешаный
			  if(Sc->Balls[i].vx>10) Sc->Balls[i].vx=9.9;
			  if(Sc->Balls[i].vx<0.5) Sc->Balls[i].vx*=2;
		 }
	}
}
void BlockHit(Scene *Sc)
{
	for(int i=0; i<Sc->row; i++)
		for(int j=0; j<Sc->column; j++)
		{
			//Проверка на то, что блок существует и то что его можно разбить
			if(Sc->AllBricks[i][j].type>0)
			{
				for(int h=0; h<Sc->num; h++)
				{
					//куча проверок на позицию
					if(Sc->Balls[h].y-Sc->Balls[h].radius<=Sc->AllBricks[i][j].ly+15)
					{
						if(Sc->Balls[h].radius+Sc->Balls[h].y>=Sc->AllBricks[i][j].ly)
						{
							if(Sc->Balls[h].x-Sc->Balls[h].radius<=Sc->AllBricks[i][j].lx+41)
							{
								if(Sc->Balls[h].radius+Sc->Balls[h].x>=Sc->AllBricks[i][j].lx)
								{
									//Проверкb на то какого типа блок для выдачи очков
									if(Sc->AllBricks[i][j].type==1) Sc->Score+=100;
									if(Sc->AllBricks[i][j].type==2) Sc->Score+=200;
									...
									//Удаление блока, изменение скорости
									if(Sc->AllBricks[i][j].type<9) Sc->AllBricks[i][j].type=0;
									Sc->Balls[h].vy*=-1.05;
									Sc->Balls[h].vx*=-1.05;
									//Черные блоки которые зарисовывают обычные, потому что ф-я клиардевайс га. тормозит
									blackbricks();
									bricks(Sc);
								}
							}
...
void Background()
{
	...
	midx=320; midy=240; maxx=639; maxy=479;
...
bool DrawLevelMenu(Scene *Sc)
{
	...
	do
	{
		if (kbhit())        
        {
            ...
            switch (key)
			{
				case '1':	if (LoadLevel("Level1.txt", Sc)) return 1;
							else return 0;
							break;
				case '2':	if (LoadLevel("Level2.txt", Sc)) return 1;
							else return 0;
							break;
				case '3':	if (LoadLevel("Level3.txt", Sc)) return 1;
							else return 0;
							break;
				case '4':	if (LoadLevel("Level4.txt", Sc)) return 1;
							else return 0;
							break;
				case '5':	if (LoadLevel("Level5.txt", Sc)) return 1;
							else return 0;
							break;
				case 'c':	if (LoadLevel("Save.txt", Sc)) return 1;
							else return 0;
							break;
				case 27:	return 0;
			}
...
void initanimMenu()
{
	...
	BOOL Active;
	DWORD ThreadId;
	hThread=CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)animMenu,(void*)&Active, 0, &ThreadId);
	 if (hThread == NULL) 
        {
...
void initmusic()
{
	...
	BOOL Active;
	DWORD ThreadId;
	hThread=CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)threadFunctionMusic,(void*)&Active, 0, &ThreadId);
		 if (hThread == NULL) 
        {
...
void PaddleAndControls(Scene *Sc)
{
	...
	if(GetAsyncKeyState(key_2)) 
		...
		if(Sc->paddleX<0) Sc->paddleX=639;
		if(Sc->paddleX>639) Sc->paddleX=1;
...
{    
	...
	//Отрисовка меню и запуск функционала
		...
		if (kbhit())        
        {
            ...
            switch (key)                
            {
				...
				case 112:	cleardevice();
							...
							{
								// Объявление количества жизней и очков, пока нет уровня сложности будет так
								cleardevice();
								//Время для таймера
								timestart = time(NULL);
								...
								while(Sc.lifes>0)
								{
									rectangle(0,0,maxx,maxy-12);
									bricks(&Sc);
									//Проверка столкновений cо стенками и проверка столкновения с низом
									Window(&Sc);
									//Проверка столкновения с блоком и его уничтожение
									BlockHit(&Sc);
									//Управление, отрисовка и проверка столкновения с пэдлом
									PaddleAndControls(&Sc);
									//Проверка столкновения
									PlatformCheck(&Sc); 
									//Просчет координат и отрисовка шара
									DrawBall(&Sc);
									//Счет
									...
									sprintf(Time, "Time: %0.0lf", difftime(current_time,timestart)+Sc.timer);
									outtextxy(30,2,Time);

									//Если кончились блоки
				...
				case 73:
				case 'i':	cleardevice();
							outtextxy(30, 10, "«Арканоид» - черно-белая видеоигра для операционной системы DOS");
							outtextxy(240, 30, "Управление: <- ->");
							outtextxy(30, 50, "Для редактирования уровней откройте один из файлов с именем level");
							break;
					//r
                case 82:  
				case 114:	cleardevice(); //records		
							LoadRecords("Records.txt");
							break;
					//Esc
				case 27:	cleardevice(); //Escape
							DrawMenu();
							break;

					//e
                case 69:
				case 101:	exit(1); //Exit
            }
        }    
	}while(true);
}